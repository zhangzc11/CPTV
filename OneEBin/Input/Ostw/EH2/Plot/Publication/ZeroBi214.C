...
const double FullRange = 1.5e-3;
const int    Nbin      = 150;
const int    At2000mm  = 100;
const int    At500mm   = 40;  // 400 us
...
void ZeroFN()
{
  ...
  string NFFile[nSet] = { "~/WORK/wangzhe/AccSubOutput/P12e_0.7MeV_1.5ms_Scaled_Bi214/H/EH1.root",
			  "~/WORK/wangzhe/AccSubOutput/P12e_0.7MeV_1.5ms_Scaled_Bi214/H/EH2.root",
			  "~/WORK/wangzhe/AccSubOutput/P12e_0.7MeV_1.5ms_Scaled_Bi214/H/EH3.root" };
  ...
  for( int s=0; s<nSet; s++ ) {
    ...
    h1dVtx[s] = (TH1F*)f.Get( "h1dDtVtxSum"  );
    h1dVtx[s]->SetDirectory(0);
    h1dBkg[s] = (TH1F*)f.Get( "h1dDtBkgSum"  );
    ...
    h1dSum[s] = (TH1F*)f.Get( "h1dDtAccSum"  );
  ...
  for( int s=0; s<nSet; s++ ) {
    ...
    cout<<SumName[s]<<" N= "<<N<<" \\pm "<<sqrt(N)<<"(sta.)"<<endl;
    cout<<SumName[s]<<" N-AB= "<<C<<" \\pm "<<CESta<<"(sta.) \\pm "<<CESys<<"(sys.)"<<endl;
  ...
  for( int s=0; s<nSet; s++ ) {
    ...
    */
    ...
    cSum[s]->SetGrid();
    h1dSumNS[s]->Draw();
    //h1dSumNS[s]->Fit("pol0","","",0.001,0.0015);
    h1dSumNS[s]->GetXaxis()->SetTitle("Lifetime [s]");
    h1dSumNS[s]->GetYaxis()->SetTitle("Entries / 1e-5 s");
  ...
  */
  ...
  for( int b=0; b<2; b++ ) {
    ...
    double V = hVtx[b]->Integral(2,topbin);
    double B = hBkg[b]->Integral(2,topbin);

    for( int bn=1; bn<=topbin; bn++ ) {

      double VCut = hVtx[b]->Integral(2,bn);
      double BCut = hBkg[b]->Integral(2,bn);

      double pV = VCut/V;
      double EVCut = sqrt( V * pV * (1-pV) );
      double EBCut = GetTotErr( hBkg[b], 2, bn );
  ...
  mg->Draw("A");
  lEffiSum->Draw();
  mg->GetXaxis()->SetTitle("Lifetime [s]");
  ...
  mgDiff->Draw("A");
  lDiffSum->Draw();
  mgDiff->GetXaxis()->SetTitle("Lifetime [s]");
...
void ZeroAll()
{
  ///////////////////////////
  const int nSet = 6;
  string SumName[nSet]   = { "AD1", "AD2", "AD3", "AD4", "AD5", "AD6" };
  ...
  // Gd
  string NFFile[nSet] = {"~/WORK/wangzhe/AccSubOutput/P12e_0.7MeV_1.5ms_Scaled_Bi214/H/EH1.root",
			 "~/WORK/wangzhe/AccSubOutput/P12e_0.7MeV_1.5ms_Scaled_Bi214/H/EH1.root",
			 "~/WORK/wangzhe/AccSubOutput/P12e_0.7MeV_1.5ms_Scaled_Bi214/H/EH2.root",
			 "~/WORK/wangzhe/AccSubOutput/P12e_0.7MeV_1.5ms_Scaled_Bi214/H/EH3.root",
			 "~/WORK/wangzhe/AccSubOutput/P12e_0.7MeV_1.5ms_Scaled_Bi214/H/EH3.root",
			 "~/WORK/wangzhe/AccSubOutput/P12e_0.7MeV_1.5ms_Scaled_Bi214/H/EH3.root"
  ...
  for( int s=0; s<nSet; s++ ) {
    TFile f( NFFile[s].c_str() );
    h1dVtx[s] = (TH1F*)f.Get( ("h1dDtVtx_"+idx[s]).c_str()  );
    h1dVtx[s]->SetDirectory(0);
    h1dBkg[s] = (TH1F*)f.Get( ("h1dDtBkg_"+idx[s]).c_str()  );
    h1dBkg[s]->SetDirectory(0);
    h1dBkgNS[s] = new TH1F( (SumName[s]+"Bkg").c_str(), (SumName[s]+"Bkg").c_str(), h1dBkg[s]->GetNbinsX(), 0, h1dBkg[s]->GetXaxis()->GetXmax());
    h1dBkgNS[s]->SetDirectory(0);
    h1dSum[s] = (TH1F*)f.Get( ("h1dDtAcc_"+idx[s]).c_str()  );
    h1dSum[s]->SetDirectory(0);
    h1dSumNS[s] = new TH1F( SumName[s].c_str(), SumName[s].c_str(), h1dBkg[s]->GetNbinsX(), 0, h1dBkg[s]->GetXaxis()->GetXmax());
    h1dSumNS[s]->SetDirectory(0);
  }
  ...
  TCanvas* c = new TCanvas( "Lifetime", "Lifetime", 700, 600 );
  gStyle->SetOptStat(0);
  gStyle->SetOptTitle(0);

  c->Divide(1,2,0.001,0.000001);

  c->cd(1);

  gPad->SetMargin(0.15,0.1,0.0,0.1);
  gPad->SetGrid();

  TLegend*  lDiffSum = new TLegend(0.67,0.28, 0.82,0.8);
  ...
  for( int b=0; b<nSet; b++ ) {
    ...
    double V = h1dVtx[b]->Integral(2,topbin);
    double B = h1dBkg[b]->Integral(2,topbin);

    for( int bn=1; bn<=topbin; bn++ ) {

      double VCut = h1dVtx[b]->Integral(2,bn);
      double BCut = h1dBkg[b]->Integral(2,bn);

      double pV = VCut/V;
      double EVCut = sqrt( V * pV * (1-pV) );
      double EBCut = GetTotErr( h1dBkg[b], 2, bn );
  ...
  /*
  TCanvas *cEffiSum = new TCanvas( "EffiAll", "EffiAll" );
  cEffiSum->SetMargin(0.15,0.15,0.1,0.1);
  TMultiGraph *mg = new TMultiGraph();
  TLegend*  lEffiSum = new TLegend(0.5,0.5,0.7,0.8);
  ...
  */
  ...
  for( int d=0; d<nSet; d++ ) {
    for( int bn=1; bn<=topbin; bn++ ) {
      ...
      DiffSum [d][bn-1] = D;
      //DiffSumE[d][bn-1] = dD;
      /* Don't draw the error. Too messy. */ 
      DiffSumE[d][bn-1] = 0;
    }
  }
  ...
  c->cd(2);
  gPad->SetMargin(0.15,0.1,0.15,0.0);
  TMultiGraph *mgDiff = new TMultiGraph();
  ...
  for( int b=0; b<nSet; b++ ) { // Start from AD 2
    ...
    mgDiff->Add( gDiffSum[b],"lp" );
  }

  mgDiff->Draw("A");

  mgDiff->GetXaxis()->SetTitle("Lifetime [s]");
  mgDiff->GetXaxis()->SetLimits(0.0000,0.0015);
  mgDiff->GetXaxis()->SetTitleSize(0.07);
  mgDiff->GetXaxis()->SetLabelSize(0.07);

  mgDiff->GetYaxis()->SetTitle("Effi. diff.");  
  mgDiff->GetYaxis()->SetRangeUser(-0.0045,0.0045);
  mgDiff->GetYaxis()->SetTitleOffset(1.);
  mgDiff->GetYaxis()->SetTitleSize(0.07);
  mgDiff->GetYaxis()->SetLabelSize(0.07);
  
  gPad->SetGridy();
  gPad->SetGridx();
  c->Print( "Lifetime.ps" );
  c->Print( "Lifetime.eps" );
}
