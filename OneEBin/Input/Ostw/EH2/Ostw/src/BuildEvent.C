...
int BuildEvent( SubEvt* pSubEvt )
{
  ...
  if( MuonEvt || GoodAdSubEvt ) {
	...
	if( MuonEvt )  {
	  ...
	  for( int FoldIdx = 0; FoldIdx < CurrEvent.Fold; FoldIdx++ )   {	    
		...
		if( CurrEvent.D2First[1] > 500 /* mm */ ||
		    CurrEvent.T2PrevSubEvt[1] > 4e-4 /* s */) {
  ...
  if( MuonEvt )   {
	...
	if(pSubEvt->E > 1500)
	{
		CurrRun.NHMuonG15[det] += 1;
		CurrRun.NHMuonG10[det] += 1;
		CurrRun.NHMuonG5[det] += 1;
	}
	else if(pSubEvt->E > 1000)
	{
		CurrRun.NHMuonG10[det] += 1;
		CurrRun.NHMuonG5[det] += 1;
	}
	else if(pSubEvt->E > 500)
	{
		CurrRun.NHMuonG5[det] += 1;
	}
	else
	{
		CurrRun.NHMuonL5[det] += 1;
	}
