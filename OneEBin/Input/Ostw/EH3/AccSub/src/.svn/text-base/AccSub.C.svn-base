...
int main(int argc, char** argv)
{
  ...
  for( unsigned int i=0; i<inputfiles.size(); i++ )  {
	...
	cout<<"Can't load Event tree entry "<<entry<<endl;
      ...
      /*
      if( EvtReader->Fold == 1 || 
	  ( EvtReader->Fold == 2 && EvtReader->D2First[1]>500 ) ) {
      */
	for( int fld=0; fld<EvtReader->Fold; fld++ )  {
	  EventBrief aEvt;
	  aEvt.E = EvtReader->E[fld];
	  aEvt.X = EvtReader->X[fld];
	  aEvt.Y = EvtReader->Y[fld];
	  aEvt.Z = EvtReader->Z[fld];
	...
	if( dist<Cut->D2First/mm 
	  ...
	  h2dAccBkg[ run ][ Det ]->Fill( Ep, Ed );
	    ...
	    /* This exp distribution doesn't work. */
	    /*
	    double t=0;
	    while( t<0.9e-6 || t>1.5e-3 ) {
	      t=r.Exp( tau );
	    }
	    */
...
int EndRun( int run ) 
{
  ...
  for( int AdNo = 1; AdNo <= 4; AdNo++ )   {
    ...
    if( bkgEntries==0 )   {
      ...
      this_scale = -NormalizationFactor/bkgEntries ;
    }
