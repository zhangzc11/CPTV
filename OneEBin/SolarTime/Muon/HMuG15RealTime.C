...
void HMuG15RealTime()
{
	...
	Double_t HMuG15InEachDay[8][707];
	Double_t E2_HMuG15InEachDay[8][707];
	memset(HMuG15InEachDay,0.0,sizeof(HMuG15InEachDay));
	...
	for(int Bin=0;Bin<NumOfBin;Bin++)
	{
		...
		for(int Det=0;Det<8;Det++)
		{
			...
			HMuG15InEachDay[Det][N707] += HMuG15_t[Det];
	...
	{
		E2_HMuG15InEachDay[Det][N707]=HMuG15InEachDay[Det][N707];
	}
	}
	...
	const char *file_Name[3] = {"HMuG15RealTime_EH1.pdf","HMuG15RealTime_EH2.pdf","HMuG15RealTime_EH3.pdf"};
	...
	for(int Det=0;Det<8;Det++)
	{
		...
		for(int i=0;i<n;i++)
	    {
			...
			{
		x[i]=i;
		y[i]=86400.0*HMuG15InEachDay[Det][i]/FullTimeInEachDay[Det][i];
		ex[i]=0;
		ey[i]=0.0;//(86400.0*HMuG15InEachDay[Det][i]/FullTimeInEachDay[Det][i])*sqrt(E2_HMuG15InEachDay[Det][i])/HMuG15InEachDay[Det][i];
			}
			else
			{
		x[i]=i;
		...
		ey[i]=0.0;
			...
			/*if(y[i]>20)
			{
				...
				//cout<<Det<<"   "<<i<<"   "<<y[i]<<"   "<<HMuG15InEachDay[Det][i]<<"   "<<FullTimeInEachDay[Det][i]<<endl;
		x[i]=i;
		y[i]=0.0;
		ex[i]=0;
		ey[i]=0.0;
			}*/
			//cout<<HMuG15InEachDay[Det][i]<<"   ";
	    }
    ...
    TCanvas *myC[3];//= new TCanvas("HMuG151","HMuG151",200,10,900,200);
    ...
    mg[i]->GetYaxis()->SetTitle("1.5~2.5GeV muon rate (/day)");
	...
	if(i==0)
	{
		mg[0]->GetYaxis()->SetRangeUser(20.0e3,22.5e3);
	}
	if(i==1)
	{
		mg[1]->GetYaxis()->SetRangeUser(17.5e3,20.5e3);
	}
	if(i==2)
	{
		mg[2]->GetYaxis()->SetRangeUser(1.5e3,2.0e3);
	}
