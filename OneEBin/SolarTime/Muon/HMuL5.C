...
void HMuL5()
{
	...
	Double_t HMuL5InOneDay[8][24];
	memset(HMuL5InOneDay,0.0,sizeof(HMuL5InOneDay));
	...
	for(int Bin=0;Bin<NumOfBin;Bin++)
	{
		...
		{
			HMuL5InOneDay[Det][N24] += HMuL5_t[Det];
			FullTimeInOneDay[Det][N24] += FullTime_t[Det];
			TotalHMuL5[Det] += HMuL5_t[Det];
		}
	}
	
	const char *hist_Name[8] = {"EH1_AD1","EH1_AD2","EH2_AD1","EH2_AD2","EH3_AD1","EH3_AD2","EH3_AD3","EH3_AD4"}; 
	const char *file_Name[8] = {
		"HMuL5InOneDay_EH1_AD1.pdf","HMuL5InOneDay_EH1_AD2.pdf","HMuL5InOneDay_EH2_AD1.pdf","HMuL5InOneDay_EH2_AD2.pdf",
		"HMuL5InOneDay_EH3_AD1.pdf","HMuL5InOneDay_EH3_AD2.pdf","HMuL5InOneDay_EH3_AD3.pdf","HMuL5InOneDay_EH3_AD4.pdf"};
	TCanvas *myC[8];
	TH1F *my_h[8];
	TLegend *leg[8];
	for(int Det=0;Det<8;Det++)
	{
	    ...
	    Double_t MaxR=0.0,MinR=86400.0*HMuL5InOneDay[Det][0]/FullTimeInOneDay[Det][0],MeanR=0.0,MaxE=0.0;

	    for(int i=0;i<24;i++)
	    {
		   my_h[Det]->SetBinContent(i+1,86400.0*HMuL5InOneDay[Det][i]/FullTimeInOneDay[Det][i]);
		   my_h[Det]->SetBinError(i+1,86400.0*sqrt(HMuL5InOneDay[Det][i])/FullTimeInOneDay[Det][i]);
		   MeanR += 86400.0*HMuL5InOneDay[Det][i]/FullTimeInOneDay[Det][i];
		   if(86400.0*HMuL5InOneDay[Det][i]/FullTimeInOneDay[Det][i]>MaxR){MaxR=86400.0*HMuL5InOneDay[Det][i]/FullTimeInOneDay[Det][i];}
		   if(86400.0*HMuL5InOneDay[Det][i]/FullTimeInOneDay[Det][i]<MinR){MinR=86400.0*HMuL5InOneDay[Det][i]/FullTimeInOneDay[Det][i];}
		   if(1.0/sqrt(HMuL5InOneDay[Det][i]) > MaxE){MaxE = 1.0/sqrt(HMuL5InOneDay[Det][i]);}
	    }
		cout<<"Total HMuL5 of "<<hist_Name[Det]<<TotalHMuL5[Det]<<endl;
	...
	my_h[Det]->GetYaxis()->SetTitle("20~500MeV Muon Rate (/day)");
   ...
   leg[Det]->Draw();
	...
	//save
	TFile  *F_HMuL5InOneDay = new TFile("HMuL5InOneDay.root","recreate");
	TTree *Tree_HMuL5InOneDay = new TTree("HMuL5InOneDay","HMuL5InOneDay");
	Tree_HMuL5InOneDay->Branch("HMuL5InOneDay",HMuL5_t,"HMuL5_t[8]/D");

	for(int Bin=0; Bin<24;Bin++)
	{
		for(int Det=0;Det<8;Det++)
		{
			HMuL5_t[Det] = HMuL5InOneDay[Det][Bin];
		}
		Tree_HMuL5InOneDay->Fill();
	}
	Tree_HMuL5InOneDay->Write();
