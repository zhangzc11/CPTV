...
void NetMu()
{
	...
	const char *file_Name[8] = {
		"NetMuInOneDay_EH1_AD1.pdf","NetMuInOneDay_EH1_AD2.pdf","NetMuInOneDay_EH2_AD1.pdf","NetMuInOneDay_EH2_AD2.pdf",
		"NetMuInOneDay_EH3_AD1.pdf","NetMuInOneDay_EH3_AD2.pdf","NetMuInOneDay_EH3_AD3.pdf","NetMuInOneDay_EH3_AD4.pdf"};
	TCanvas *myC[8];
	TH1F *my_h[8];
	TLegend *leg[8];
	for(int Det=0;Det<8;Det++)
	{
	    ...
	    Double_t MaxR=0.0,MinR=86400.0*NetMuInOneDay[Det][0]/FullTimeInOneDay[Det][0],MeanR=0.0,MaxE=0.0;

	    for(int i=0;i<24;i++)
	    {
		   my_h[Det]->SetBinContent(i+1,86400.0*NetMuInOneDay[Det][i]/FullTimeInOneDay[Det][i]);
		   my_h[Det]->SetBinError(i+1,86400.0*sqrt(NetMuInOneDay[Det][i])/FullTimeInOneDay[Det][i]);
		   MeanR += 86400.0*NetMuInOneDay[Det][i]/FullTimeInOneDay[Det][i];
		   if(86400.0*NetMuInOneDay[Det][i]/FullTimeInOneDay[Det][i]>MaxR){MaxR=86400.0*NetMuInOneDay[Det][i]/FullTimeInOneDay[Det][i];}
		   if(86400.0*NetMuInOneDay[Det][i]/FullTimeInOneDay[Det][i]<MinR){MinR=86400.0*NetMuInOneDay[Det][i]/FullTimeInOneDay[Det][i];}
		   if(1.0/sqrt(NetMuInOneDay[Det][i]) > MaxE){MaxE = 1.0/sqrt(NetMuInOneDay[Det][i]);}
	    }
		cout<<"Total NetMu of "<<hist_Name[Det]<<TotalNetMu[Det]<<endl;
	...
	my_h[Det]->GetYaxis()->SetTitle("Net Muon Rate (/day)");
   ...
   leg[Det]->Draw();
	...
	//save
	TFile  *F_NetMuInOneDay = new TFile("NetMuInOneDay.root","recreate");
	TTree *Tree_NetMuInOneDay = new TTree("NetMuInOneDay","NetMuInOneDay");
	Tree_NetMuInOneDay->Branch("NetMuInOneDay",NetMu_t,"NetMu_t[8]/D");

	for(int Bin=0; Bin<24;Bin++)
	{
		for(int Det=0;Det<8;Det++)
		{
			NetMu_t[Det] = NetMuInOneDay[Det][Bin];
		}
		Tree_NetMuInOneDay->Fill();
	}
	Tree_NetMuInOneDay->Write();
