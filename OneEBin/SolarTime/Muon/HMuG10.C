...
void HMuG10()
{
	...
	Double_t HMuG10InOneDay[8][24];
	memset(HMuG10InOneDay,0.0,sizeof(HMuG10InOneDay));
	...
	for(int Bin=0;Bin<NumOfBin;Bin++)
	{
		...
		{
			HMuG10InOneDay[Det][N24] += HMuG10_t[Det];
			FullTimeInOneDay[Det][N24] += FullTime_t[Det];
			TotalHMuG10[Det] += HMuG10_t[Det];
		}
	}
	
	const char *hist_Name[8] = {"EH1_AD1","EH1_AD2","EH2_AD1","EH2_AD2","EH3_AD1","EH3_AD2","EH3_AD3","EH3_AD4"}; 
	const char *file_Name[8] = {
		"HMuG10InOneDay_EH1_AD1.pdf","HMuG10InOneDay_EH1_AD2.pdf","HMuG10InOneDay_EH2_AD1.pdf","HMuG10InOneDay_EH2_AD2.pdf",
		"HMuG10InOneDay_EH3_AD1.pdf","HMuG10InOneDay_EH3_AD2.pdf","HMuG10InOneDay_EH3_AD3.pdf","HMuG10InOneDay_EH3_AD4.pdf"};
	TCanvas *myC[8];
	TH1F *my_h[8];
	TLegend *leg[8];
	for(int Det=0;Det<8;Det++)
	{
		...
		Double_t MaxR=0.0,MinR=86400.0*HMuG10InOneDay[Det][0]/FullTimeInOneDay[Det][0],MeanR=0.0,MaxE=0.0;

	    for(int i=0;i<24;i++)
	    {
		   my_h[Det]->SetBinContent(i+1,86400.0*HMuG10InOneDay[Det][i]/FullTimeInOneDay[Det][i]);
		   my_h[Det]->SetBinError(i+1,86400.0*sqrt(HMuG10InOneDay[Det][i])/FullTimeInOneDay[Det][i]);
		   MeanR += 86400.0*HMuG10InOneDay[Det][i]/FullTimeInOneDay[Det][i];
		   if(86400.0*HMuG10InOneDay[Det][i]/FullTimeInOneDay[Det][i]>MaxR){MaxR=86400.0*HMuG10InOneDay[Det][i]/FullTimeInOneDay[Det][i];}
		   if(86400.0*HMuG10InOneDay[Det][i]/FullTimeInOneDay[Det][i]<MinR){MinR=86400.0*HMuG10InOneDay[Det][i]/FullTimeInOneDay[Det][i];}
		   if(1.0/sqrt(HMuG10InOneDay[Det][i]) > MaxE){MaxE = 1.0/sqrt(HMuG10InOneDay[Det][i]);}
	    }
		cout<<"Total HMuG10 of "<<hist_Name[Det]<<TotalHMuG10[Det]<<endl;
	...
	my_h[Det]->GetYaxis()->SetTitle("1000~2500MeV Muon Rate (/day)");
   ...
   leg[Det]->Draw();
	...
	//save
	TFile  *F_HMuG10InOneDay = new TFile("HMuG10InOneDay.root","recreate");
	TTree *Tree_HMuG10InOneDay = new TTree("HMuG10InOneDay","HMuG10InOneDay");
	Tree_HMuG10InOneDay->Branch("HMuG10InOneDay",HMuG10_t,"HMuG10_t[8]/D");

	for(int Bin=0; Bin<24;Bin++)
	{
		for(int Det=0;Det<8;Det++)
		{
			HMuG10_t[Det] = HMuG10InOneDay[Det][Bin];
		}
		Tree_HMuG10InOneDay->Fill();
	}
	Tree_HMuG10InOneDay->Write();
