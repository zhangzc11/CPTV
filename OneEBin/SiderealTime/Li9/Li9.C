...
//	Int_t NumOfBin = (EndTime - StartTime)/WidthOfBin;//17016
	...
	double Li9InEachBin[8][NumOfBin]={{0.0}};
	double RD_Li9InEachBin[8][NumOfBin]={{0.0}};
	...
	for(int Det=0;Det<8;Det++)
	{
		...
		{
			Li9InOneDay[Det][N24] = TotalLi9[Det] * HMuG5InOneDay[Det][N24]/TotalHMuG5[Det];
			RD_Li9InOneDay[Det][N24] = sqrt(1.0/HMuG5InOneDay[Det][N24] - 1.0/TotalHMuG5[Det]);
		}
		for(int N24=0;N24<NumOfBin;N24++)
		{
			if(FullTimeInEachBin[Det][N24]>10)
			{
			Li9InEachBin[Det][N24] = TotalLi9[Det] * HMuG5InEachBin[Det][N24]/TotalHMuG5[Det];
			if(HMuG5InEachBin[Det][N24]>1.0)
			{
			RD_Li9InEachBin[Det][N24] = sqrt(1.0/HMuG5InEachBin[Det][N24] - 1.0/TotalHMuG5[Det]);
			}
			if(RD_Li9InEachBin[Det][N24]>1.0)
			{
				Li9InEachBin[Det][N24] = 0.0;
				RD_Li9InEachBin[Det][N24]=0.0;
			}
			}
		}
	}
	...
	my_h[Det]->GetYaxis()->SetTitle("9Li/8He Rate (/day)");
   ...
   leg[Det]->Draw();
	...
	//save in each bin
	TFile  *F_Li9InEachBin = new TFile("Li9InEachBin.root","recreate");
	TTree *Tree_Li9InEachBin = new TTree("Li9InEachBin","Li9InEachBin");
	Tree_Li9InEachBin->Branch("Li9InEachBin",Li9_t,"Li9_t[8]/D");

	for(int Bin=0; Bin<NumOfBin;Bin++)
	{
		for(int Det=0;Det<8;Det++)
		{
			Li9_t[Det] = Li9InEachBin[Det][Bin];
		}
		Tree_Li9InEachBin->Fill();
	}
	Tree_Li9InEachBin->Write();

	TFile  *F_RD_Li9InEachBin = new TFile("RD_Li9InEachBin.root","recreate");
	TTree *Tree_RD_Li9InEachBin = new TTree("RD_Li9InEachBin","RD_Li9InEachBin");
	Tree_RD_Li9InEachBin->Branch("RD_Li9InEachBin",Li9_t,"Li9_t[8]/D");

	for(int Bin=0; Bin<NumOfBin;Bin++)
	{
		for(int Det=0;Det<8;Det++)
		{
			Li9_t[Det] = RD_Li9InEachBin[Det][Bin];
		}
		Tree_RD_Li9InEachBin->Fill();
	}
	Tree_RD_Li9InEachBin->Write();
