...
//	Int_t NumOfBin = (EndTime - StartTime)/WidthOfBin;//17016
	...
	const char *file_Name[8] = {
		"AccRate_EH1_AD1.eps","AccRate_EH1_AD2.eps","AccRate_EH2_AD1.eps","AccRate_EH2_AD2.eps",
		"AccRate_EH3_AD1.eps","AccRate_EH3_AD2.eps","AccRate_EH3_AD3.eps","AccRate_EH3_AD4.eps"};
	TCanvas *myC[8];
	TH1F *my_h[8];
	TLegend *leg[8];
	for(int Det=0;Det<8;Det++)
	...
	if(Det==4)
		{
	my_h[Det]->GetYaxis()->SetRangeUser(1.58,1.68);
		}
	...
	my_h[Det]->GetYaxis()->SetTitleOffset(0.6);
   ...
   //leg[Det]->Draw();
	...
	//save
	TFile  *F_AccInOneDay = new TFile("AccRateInOneDay.root","recreate");
	...
	for(int Bin=0; Bin<24;Bin++)
	{
		for(int Det=0;Det<8;Det++)
		{
			Acc_t[Det] = 86400.0*AccInOneDay[Det][Bin]/FullTimeInOneDay[Det][Bin];// in unit of events/day	
		}
		Tree_AccInOneDay->Fill();
	}
	Tree_AccInOneDay->Write();

	TFile  *F_Error_AccInOneDay = new TFile("Error_AccRateInOneDay.root","recreate");
	TTree *Tree_Error_AccInOneDay = new TTree("Error_AccInOneDay","Error_AccInOneDay");
	Tree_Error_AccInOneDay->Branch("Error_AccInOneDay",Acc_t,"Acc_t[8]/D");

	for(int Bin=0; Bin<24;Bin++)
	{
		for(int Det=0;Det<8;Det++)
		{
			Acc_t[Det] = RD_AccInOneDay[Det][Bin]*86400.0*AccInOneDay[Det][Bin]/FullTimeInOneDay[Det][Bin];
		}
		Tree_Error_AccInOneDay->Fill();
	}
	Tree_Error_AccInOneDay->Write();
