...
//	Int_t NumOfBin = (EndTime - StartTime)/WidthOfBin;//17016
	...
	double SgUpInEachBin[8][NumOfBin] = {{0.0}};
	double SgLowInEachBin[8][NumOfBin] = {{0.0}};
	double NetMuInEachBin[8][NumOfBin] = {{0.0}};
	double MultiEffInEachBin[8][NumOfBin] = {{0.0}};
	double E_MultiEffInEachBin[8][NumOfBin] = {{0.0}};
	...
	for(int Bin=0;Bin<NumOfBin;Bin++)
	{
		...
		for(int Det=0;Det<8;Det++)
		{
			...
			SgUpInEachBin[Det][Bin] = SgUp_t[Det];
			SgLowInEachBin[Det][Bin] = SgLow_t[Det];
			NetMuInEachBin[Det][Bin] = NetMu_t[Det];
	...
	for(int Det=0;Det<8;Det++)
	{
		...
		{
			if((FullTimeInEachBin[Det][N24]>20)&&(LiveTimeInEachBin[Det][N24]>20))
			{
			MultiEffInEachBin[Det][N24] = f_MultiEff(NetMuInEachBin[Det][N24]/FullTimeInEachBin[Det][N24],0.5*(SgUpInEachBin[Det][N24]+SgLowInEachBin[Det][N24])/LiveTimeInEachBin[Det][N24],Tc);
			E_MultiEffInEachBin[Det][N24] = f_MultiEff_Div(NetMuInEachBin[Det][N24]/FullTimeInEachBin[Det][N24],NetMuInEachBin[Det][N24],0.5*(SgUpInEachBin[Det][N24]+SgLowInEachBin[Det][N24])/LiveTimeInEachBin[Det][N24],0.5*(SgUpInEachBin[Det][N24]+SgLowInEachBin[Det][N24]),Tc);
			//cout<<Det<<"   "<<N24<<"   "<<MultiEffInEachBin[Det][N24]<<endl;
			}
		}
	...
	const char *file_Name[8] = {
		"MultiEffInOneDay_EH1_AD1.eps","MultiEffInOneDay_EH1_AD2.eps","MultiEffInOneDay_EH2_AD1.eps","MultiEffInOneDay_EH2_AD2.eps",
		"MultiEffInOneDay_EH3_AD1.eps","MultiEffInOneDay_EH3_AD2.eps","MultiEffInOneDay_EH3_AD3.eps","MultiEffInOneDay_EH3_AD4.eps"};
	TCanvas *myC[8];
	TH1F *my_h[8];
	TLegend *leg[8];
	for(int Det=0;Det<8;Det++)
	...
	if(Det==4)
		{
	my_h[Det]->GetYaxis()->SetRangeUser(0.97772,0.97782);
		}
   ...
   //leg[Det]->Draw();
	...
	//save in each bin
	TFile  *F_MultiEffInEachBin = new TFile("MultiEffInEachBin.root","recreate");
	TTree *Tree_MultiEffInEachBin = new TTree("MultiEffInEachBin","MultiEffInEachBin");
	Tree_MultiEffInEachBin->Branch("MultiEffInEachBin",MultiEff_t,"MultiEff_t[8]/D");

	for(int Bin=0; Bin<NumOfBin;Bin++)
	{
		for(int Det=0;Det<8;Det++)
		{
			MultiEff_t[Det] = MultiEffInEachBin[Det][Bin];
		}
		Tree_MultiEffInEachBin->Fill();
	}
	Tree_MultiEffInEachBin->Write();
