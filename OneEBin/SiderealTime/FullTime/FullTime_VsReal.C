...
void FullTime_VsReal()
{
	...
	Double_t FullTimeVsRealTime[8][NumOfDay];
	memset(FullTimeVsRealTime,0.0,sizeof(FullTimeVsRealTime));
	...
	for(int Bin=0;Bin<NumOfBin;Bin++)
	{
		...
		{
			FullTimeVsRealTime[Det][N24] += FullTime_t[Det];
	...
	const char *file_Name[8] = {
		"FullTimeVsRealTime_EH1_AD1.png","FullTimeVsRealTime_EH1_AD2.png","FullTimeVsRealTime_EH2_AD1.png","FullTimeVsRealTime_EH2_AD2.png",
		"FullTimeVsRealTime_EH3_AD1.png","FullTimeVsRealTime_EH3_AD2.png","FullTimeVsRealTime_EH3_AD3.png","FullTimeVsRealTime_EH3_AD4.png"};
	TCanvas *myC[8];
	TH1F *my_h[8];
	TLegend *leg[8];
	for(int Det=0;Det<8;Det++)
	{
		...
		my_h[Det] = new TH1F(hist_Name[Det],hist_Name[Det],NumOfDay,0,NumOfDay);
	    for(int i=0;i<NumOfDay;i++)
	    {
		   my_h[Det]->SetBinContent(i+1,FullTimeVsRealTime[Det][i]);
	    }
		//cout<<"Total FullTime of "<<hist_Name[Det]<<" (days):   "<<TotalFullTime[Det]/86400.0<<endl;

		my_h[Det]->GetXaxis()->SetTitle("sidereal days since T0");
	my_h[Det]->GetYaxis()->SetTitle("DAQ Livetime /seconds");
	//my_h[Det]->GetYaxis()->SetRangeUser(1990*1e3,2080*1e3);
	my_h[Det]->GetXaxis()->SetRangeUser(0,NumOfDay);
	my_h[Det]->GetXaxis()->SetNdivisions(NumOfDay);
