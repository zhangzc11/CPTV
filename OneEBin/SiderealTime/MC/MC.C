...
double LatHall[3] = {22.598,22.606,22.612};//latitude of the three halls, degree, read from goole maps
	...
	double IBD_Frac_Exp_Hall[3][24]={0.0};
	double IBD_Hall_Bin_Exp[3][24]={0.0};
	double IBD_Total_Hall[3]={0.0};
	double Frac_Weight[3][24]={0.0};
...
void MC()
{
	...
	double Frac_Weight_Total[3]={0.0};
	...
	for(int N24=0;N24<24;N24++)
	{
		...
		for(int Hall=0;Hall<3;Hall++)
		{
			for(int Rct=0;Rct<6;Rct++)
			{
				...
				Frac_Weight[Hall][N24] += Fraction_t[Hall][Rct];
				Frac_Weight_Total[Hall] += Fraction_t[Hall][Rct];
			}
		}
	}
	...
	for(int N24=0;N24<24;N24++)
	{
		for(int Hall=0;Hall<3;Hall++)
		{
			Frac_Weight[Hall][N24] = Frac_Weight[Hall][N24]/Frac_Weight_Total[Hall];

			if(N24!=0)
			{
				Frac_Weight[Hall][N24] += Frac_Weight[Hall][N24-1];
			}
		}
	}
	...
	{
		IBD_Total_Hall[0] += IBD_Total[Det];
		for(int N24=0;N24<24;N24++)
		{
		IBD_Hall_Bin_Exp[0][N24] += IBD_AD_Bin_Exp[Det][N24];
		}
	}

	for(int Det=2;Det<4;Det++)
	{
		IBD_Total_Hall[1] += IBD_Total[Det];
		for(int N24=0;N24<24;N24++)
		{
		IBD_Hall_Bin_Exp[1][N24] += IBD_AD_Bin_Exp[Det][N24];
		}
	}

	for(int Det=4;Det<8;Det++)
	{
		IBD_Total_Hall[2] += IBD_Total[Det];
		for(int N24=0;N24<24;N24++)
		{
		IBD_Hall_Bin_Exp[2][N24] += IBD_AD_Bin_Exp[Det][N24];
		}
	}
	...
	for(int Hall=0;Hall<3;Hall++)
	{
		for(int N24=0;N24<24;N24++)
		{
			IBD_Frac_Exp_Hall[Hall][N24] = IBD_Hall_Bin_Exp[Hall][N24]/IBD_Total_Hall[Hall];
		}
	}
	//////////////EH1////////////////////////////////////////////////
	int HallID = 0;
	...
	for(int N24=0;N24<24;N24++)
	{
		...
		cout<<"EH1 Power........."<<sqrt(temp2/pow(temp,2.0))<<endl;
	...
	for(int N24=0;N24<24;N24++)
	{
		...
		cout<<"EH2 Power........."<<sqrt(temp2/pow(temp,2.0))<<endl;
	...
	/*
	//RandIBD();
	double MC_IBD_Hall_Bin_t[3][24];
	const int nSample = 1000;

	TFile  *F_MC_IBD_Hall_Bin = new TFile("MC_IBD_Hall_Bin.root","recreate");
	TTree *Tree_MC_IBD_Hall_Bin = new TTree("MC_IBD_Hall_Bin","MC_IBD_Hall_Bin");
	Tree_MC_IBD_Hall_Bin->Branch("MC_IBD_Hall_Bin",MC_IBD_Hall_Bin_t,"MC_IBD_Hall_Bin_t[3][24]/D");
	...
	{
		cout<<iS<<endl;
		memset(MC_IBD_Hall_Bin_t,0.0,sizeof(MC_IBD_Hall_Bin_t));

	for(int Hall=0;Hall<3;Hall++)
	{
		//cout<<"Det: "<<Det<<endl;
		int TotalN = int(IBD_Total_Hall[Hall]);
		...
		for(int i_IBD=0;i_IBD<TotalN;i_IBD++)
		{
			...
			for(int N24=0;N24<24;N24++)
			{
				if(this_Time>Frac_Weight[Hall][N24])
				{
					this_TimeBin++;
				}
			}
			if(this_TimeBin>23){this_TimeBin=23;}
			MC_IBD_Hall_Bin_t[Hall][this_TimeBin] += 1.0;
		}

		
	}
	
	Tree_MC_IBD_Hall_Bin->Fill();

	}

	Tree_MC_IBD_Hall_Bin->Write();
	*/
}
