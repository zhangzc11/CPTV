...
double TotalIBD_Hall[3] = {306214.0+310759.0,288258.0+190533.0,41224.0+41434.0+40952.0+27560};
...
void FFT_MC_Gap()
{
	...
	FILE* m_outfile = fopen("log_MC_Gap.txt", "w+");
...
//			if(MultiEff_t_Bin[Det]*MuEff_t_Bin[Det]>0.01)
	...
	for(int idx=0;idx<nSample;idx++)
	{
		...
		//	if(!IsGoodBin[Det][Bin])
			{
				M[0] += my_Random->PoissonD(TotalIBD[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_Eff) 
					- (TotalAcc[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_Acc[Det])
					- (TotalFN[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_FN[Det])
					- (TotalAmC[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_AmC[Det])
					- (TotalLi9[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_Li9[Det]);
				P[0] += TotalIBD[Det]/NumOfGoodBin[Det]*my_Random->Gaus(1.0,RD_Power);//*0.572);
			}
		}
		for(int Det=2;Det<4;Det++)
		{
//			if(!IsGoodBin[Det][Bin])
			{
				M[1] += my_Random->PoissonD(TotalIBD[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_Eff) 
					- (TotalAcc[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_Acc[Det])
					- (TotalFN[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_FN[Det])
					- (TotalAmC[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_AmC[Det])
					- (TotalLi9[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_Li9[Det]);
				P[1] += TotalIBD[Det]/NumOfGoodBin[Det]*my_Random->Gaus(1.0,RD_Power);//*0.472);
			}
		...
		//	if(!IsGoodBin[Det][Bin])
			{
					M[2] += my_Random->PoissonD(TotalIBD[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_Eff) 
					- (TotalAcc[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_Acc[Det])
					- (TotalFN[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_FN[Det])
					- (TotalAmC[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_AmC[Det])
					- (TotalLi9[Det]/NumOfGoodBin[Det])*my_Random->Gaus(1.0,RD_Li9[Det]);
		...
		for(int Hidx=0;Hidx<3;Hidx++)
		{
			if((R_data[Hidx][Bin]<0.01)&&(M[Hidx]>0.00001))
			{
				R[Hidx][Bin] = M[Hidx]/P[Hidx];
			}
			 else
			 {
				 R[Hidx][Bin] =0.0;// my_Random->Gaus(0,1.0/sqrt(TotalIBD_Hall[Hidx]/NumOfGoodBin[Det]));
			 }
...
/*			mean_an[Hidx][Bin] = mean_an[Hidx][Bin]/mean_an[Hidx][0];
	...
	const char* file_Name_Rt[3] = {"EH1_Rt_Gap.eps","EH2_Rt_Gap.eps","EH3_Rt_Gap.eps"};
	TCanvas *myC_Rt[3];
	TGraphErrors *gr_Rt[3];

	TLegend *leg_Rt[3];
	const char* leg_Name_Rt[3] = {"EH1_R(t)-R0_MC(gap only)","EH2_R(t)-R0_MC(gap only)","EH3_R(t)-R0_MC(gap only)"};
	...
	for(int Hidx=0;Hidx<3;Hidx++)
	{
		...
		leg_Rt[Hidx] = new TLegend(0.10,0.65,0.90,0.98);
...
//Draw, _abn
	...
	const char* file_Name_abn[3] = {"EH1_abn_MC_Gap.eps","EH2_abn_MC_Gap.eps","EH3_abn_MC_Gap.eps"};
	TCanvas *myC_abn[3];

	TGraphErrors *gr_an[3];

	TLegend *leg_an[3];
	const char* leg_Name_an[3] = {"EH1_a_{n}_MC(gap only)","EH2_a_{n}_MC(gap only)","EH3_a_{n}_MC(gap only)"};
	TGraphErrors *gr_bn[3];

	TLegend *leg_bn[3];
	const char* leg_Name_bn[3] = {"EH1_b_{n}_MC(gap only)","EH2_b_{n}_MC(gap only)","EH3_b_{n}_MC(gap only)"};
	...
	for(int Hidx=0;Hidx<3;Hidx++)
	{
		...
		leg_an[Hidx] = new TLegend(0.10,0.65,0.90,0.98);
		...
		leg_bn[Hidx] = new TLegend(0.10,0.65,0.90,0.98);
	...
	const char* hist_Name_abn_NE[3] = {"EH1_abn_NE","EH2_abn_NE","EH3_abn_NE"};

	const char* file_Name_abn_NE[3] = {"EH1_abn_NE_MC_Gap.eps","EH2_abn_NE_MC_Gap.eps","EH3_abn_NE_MC_Gap.eps"};
	TCanvas *myC_abn_NE[3];

	TGraphErrors *gr_an_NE[3];

	TLegend *leg_an_NE[3];
	const char* leg_Name_an_NE[3] = {"EH1_a_{n}_MC(gap only)","EH2_a_{n}_MC(gap only)","EH3_a_{n}_MC(gap only)"};
	TGraphErrors *gr_bn_NE[3];

	TLegend *leg_bn_NE[3];
	const char* leg_Name_bn_NE[3] = {"EH1_b_{n}_MC(gap only)","EH2_b_{n}_MC(gap only)","EH3_b_{n}_MC(gap only)"};


	for(int Hidx=0;Hidx<3;Hidx++)
	{

		myC_abn_NE[Hidx] = new TCanvas(hist_Name_abn_NE[Hidx],hist_Name_abn_NE[Hidx],0,0,800,600);
		myC_abn_NE[Hidx]->Divide(1,2,0);
		{
		myC_abn_NE[Hidx]->cd(1);
		...
		y[Bin] = mean_an[Hidx][kM];
		ex[Bin]=0.0;
		ey[Bin]=0.0;//sigma_an_NE[Hidx][kM];
		}
		gr_an_NE[Hidx] = new TGraphErrors(nX,x,y,ex,ey);
		gr_an_NE[Hidx]->GetYaxis()->SetRangeUser(-0.015,0.015);
		gr_an_NE[Hidx]->GetXaxis()->SetRangeUser(0,nX/nBins);
		gr_an_NE[Hidx]->SetMarkerColor(kBlue);
		gr_an_NE[Hidx]->SetLineColor(kBlue);
		gr_an_NE[Hidx]->SetLineWidth(2);
		gr_an_NE[Hidx]->SetMarkerStyle(8);
		gr_an_NE[Hidx]->SetMarkerSize(0.5);
		gr_an_NE[Hidx]->GetXaxis()->SetTitle("#omega/#omega_{0}, #omega_{0}=2#pi/1 sidereal day");
		gr_an_NE[Hidx]->GetYaxis()->SetTitle("a");

		gr_an_NE[Hidx]->SetTitle("");

		gr_an_NE[Hidx]->GetYaxis()->SetTitleOffset(0.4);
		gr_an_NE[Hidx]->GetYaxis()->SetTitleSize(0.08);
		gr_an_NE[Hidx]->GetYaxis()->SetLabelSize(0.06);
		gr_an_NE[Hidx]->GetYaxis()->SetLabelOffset(0.0);
	
		gr_an_NE[Hidx]->GetXaxis()->SetTitleOffset(0.7);
		gr_an_NE[Hidx]->GetXaxis()->SetTitleSize(0.07);
		gr_an_NE[Hidx]->GetXaxis()->SetLabelSize(0.05);
		gr_an_NE[Hidx]->GetXaxis()->SetLabelOffset(0.0);
		gr_an_NE[Hidx]->Draw("APZ");

		leg_an_NE[Hidx] = new TLegend(0.10,0.65,0.90,0.98);
		leg_an_NE[Hidx]->AddEntry(gr_an_NE[Hidx],leg_Name_an_NE[Hidx],"");
		leg_an_NE[Hidx]->SetFillStyle(0);
		leg_an_NE[Hidx]->SetBorderSize(0);
		leg_an_NE[Hidx]->SetTextColor(kRed);
		leg_an_NE[Hidx]->Draw();
		}

	
		{
		myC_abn_NE[Hidx]->cd(2);
		...
		y[Bin] = mean_bn[Hidx][kM];
		ex[Bin]=0.0;
		ey[Bin]=0.0;//sigma_bn_NE[Hidx][kM];
		}
		gr_bn_NE[Hidx] = new TGraphErrors(nX,x,y,ex,ey);
		gr_bn_NE[Hidx]->GetYaxis()->SetRangeUser(-0.015,0.015);
		gr_bn_NE[Hidx]->GetXaxis()->SetRangeUser(0,nX/nBins);
		gr_bn_NE[Hidx]->SetMarkerColor(kBlue);
		gr_bn_NE[Hidx]->SetLineColor(kBlue);
		gr_bn_NE[Hidx]->SetLineWidth(2);
		gr_bn_NE[Hidx]->SetMarkerStyle(8);
		gr_bn_NE[Hidx]->SetMarkerSize(0.5);
		gr_bn_NE[Hidx]->GetXaxis()->SetTitle("#omega/#omega_{0}, #omega_{0}=2#pi/1 sidereal day");
		gr_bn_NE[Hidx]->GetYaxis()->SetTitle("b");

		gr_bn_NE[Hidx]->SetTitle("");

		gr_bn_NE[Hidx]->GetYaxis()->SetTitleOffset(0.4);
		gr_bn_NE[Hidx]->GetYaxis()->SetTitleSize(0.08);
		gr_bn_NE[Hidx]->GetYaxis()->SetLabelSize(0.06);
		gr_bn_NE[Hidx]->GetYaxis()->SetLabelOffset(0.0);
	
		gr_bn_NE[Hidx]->GetXaxis()->SetTitleOffset(0.7);
		gr_bn_NE[Hidx]->GetXaxis()->SetTitleSize(0.07);
		gr_bn_NE[Hidx]->GetXaxis()->SetLabelSize(0.05);
		gr_bn_NE[Hidx]->GetXaxis()->SetLabelOffset(0.0);
		gr_bn_NE[Hidx]->Draw("APZ");
	
		leg_bn_NE[Hidx] = new TLegend(0.10,0.65,0.90,0.98);
		leg_bn_NE[Hidx]->AddEntry(gr_bn_NE[Hidx],leg_Name_bn_NE[Hidx],"");
		leg_bn_NE[Hidx]->SetFillStyle(0);
		leg_bn_NE[Hidx]->SetBorderSize(0);
		leg_bn_NE[Hidx]->SetTextColor(kRed);
		leg_bn_NE[Hidx]->Draw();
		}
	
		myC_abn_NE[Hidx]->SaveAs(file_Name_abn_NE[Hidx]);
}
...
//save to .root file
	 ...
	 double an_t[3][2];
	 TFile  *F_an = new TFile("an_MC_Gap.root","recreate");
	 ...
	 double bn_t[3][2];
	 TFile  *F_bn = new TFile("bn_MC_Gap.root","recreate");
