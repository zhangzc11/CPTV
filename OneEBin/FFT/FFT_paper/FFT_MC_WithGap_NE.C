...
double AVEUnc_Hall[3] = {0.0};

void FFT_MC_WithGap_NE()
{
...
//	{
//		AVEUnc_Hall[Hidx]  += 1.0/TotalIBD_Hall[Hidx]/NumOfBin;
//		AVEUnc_Hall[Hidx]  += RD_Eff*RD_Eff;
//	}
//	AVEUnc_Hall[0] += 
	//cout<<"hello......"<<endl;
	...
	FILE* m_outfile = fopen("log_MC_WithGap.txt", "w+");
...
//			if(MultiEff_t_Bin[Det]*MuEff_t_Bin[Det]>0.01)
	...
	cout<<"AVEUnc_Hall:   "<<AVEUnc_Hall[0]<<"   "<<AVEUnc_Hall[1]<<"   "<<AVEUnc_Hall[2]<<endl;
...
//			if(!IsGoodBin[Det][Bin])
		...
		for(int Hidx=0;Hidx<3;Hidx++)
		{
			...
			{
				R[Hidx][Bin] = 0.0;//my_Random->Gaus(0.0,AVEUnc_Hall[Hidx]);
			}
		}
		}
...
//	//cout<<"...................."<<mean_an[0][0]<<"  "<<sigma_an[0][0]<<endl;
	...
	for(int Hidx=0;Hidx<3;Hidx++)
	{
		for(int Bin=0;Bin<NumOfBin;Bin++)
		{
			...
			if(sigma_an[Hidx][Bin]<0)
			{
				sigma_an[Hidx][Bin] = 0.0;
			}
			else
			{
  ...
  //Draw, Rt
	...
	const char* file_Name_Rt[3] = {"EH1_Rt_WithGap_NE.eps","EH2_Rt_WithGap_NE.eps","EH3_Rt_WithGap_NE.eps"};
	TCanvas *myC_Rt[3];
	TGraphErrors *gr_Rt[3];

	TLegend *leg_Rt[3];
	const char* leg_Name_Rt[3] = {"EH1_R(t)-R0_MC(with gap)","EH2_R(t)-R0_MC(with gap)","EH3_R(t)-R0_MC(with gap)"};
...
//Draw, _abn
	...
	const char* file_Name_abn[3] = {"EH1_abn_MC_WithGap_NE.eps","EH2_abn_MC_WithGap_NE.eps","EH3_abn_MC_WithGap_NE.eps"};
	TCanvas *myC_abn[3];

	TGraphErrors *gr_an[3];

	TLegend *leg_an[3];
	const char* leg_Name_an[3] = {"EH1_a_{n}_MC(with gap)","EH2_a_{n}_MC(with gap)","EH3_a_{n}_MC(with gap)"};
	TGraphErrors *gr_bn[3];

	TLegend *leg_bn[3];
	const char* leg_Name_bn[3] = {"EH1_b_{n}_MC(with gap)","EH2_b_{n}_MC(with gap)","EH3_b_{n}_MC(with gap)"};
...
//save to .root file
	 ...
	 double an_t[3][2];
	 TFile  *F_an = new TFile("an_MC_WithGap.root","recreate");
	 ...
	 double bn_t[3][2];
	 TFile  *F_bn = new TFile("bn_MC_WithGap.root","recreate");
