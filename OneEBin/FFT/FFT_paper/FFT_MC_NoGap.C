...
void FFT_MC_NoGap()
{
	...
	FILE* m_outfile = fopen("log_MC_NoGap.txt", "w+");
...
//			if(!IsGoodBin[Det][Bin])
   ...
   //Draw, Rt
	...
	TLegend *leg_Rt[3];
	const char* leg_Name_Rt[3] = {"EH1_R(t)-R0_MC(no gap)","EH2_R(t)-R0_MC(no gap)","EH3_R(t)-R0_MC(no gap)"};
	...
	for(int Hidx=0;Hidx<3;Hidx++)
	{
		...
		leg_Rt[Hidx] = new TLegend(0.1, 0.65, 0.9, 0.98);
...
//Draw, _abn
	...
	TLegend *leg_an[3];
	const char* leg_Name_an[3] = {"EH1_a_{n}_MC(no gap)","EH2_a_{n}_MC(no gap)","EH3_a_{n}_MC(no gap)"};
	TGraphErrors *gr_bn[3];

	TLegend *leg_bn[3];
	const char* leg_Name_bn[3] = {"EH1_b_{n}_MC(no gap)","EH2_b_{n}_MC(no gap)","EH3_b_{n}_MC(no gap)"};
	...
	for(int Hidx=0;Hidx<3;Hidx++)
	{
		...
		leg_an[Hidx] = new TLegend(0.1, 0.65, 0.9, 0.98);
...
//save to .root file
	 ...
	 double an_t[3][2];
	 TFile  *F_an = new TFile("an_MC_NoGap.root","recreate");
	 ...
	 double bn_t[3][2];
	 TFile  *F_bn = new TFile("bn_MC_NoGap.root","recreate");
