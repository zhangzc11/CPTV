...
const int NumOfBin = 13824;//16944, gap removed
...
double TotalAcc[8] = {4960.35,4928.85,3781.7,2507.12,915.929,859.044,849.355,443.084};
double TotalAmC[8] = {126.276,126.161,119.177,83.2495,64.0152,54.6572,54.6703,26.088};
double TotalFN[8] = {441.04,441.04,306.73,204.34,28.1207,28.1207,28.1207,18.6343};
double TotalLi9[8] = {1583.22,1583.22,965.631,643.292,151.852,151.852,151.852,100.625};
...
//			if(!IsGoodBin[Det][Bin])
	...
	for(int Hidx=0;Hidx<3;Hidx++)
	{
		...
		FILE* m_outfile = fopen("log_MC_GapRemove.txt", "w+");
	//print
	for(int Bin=0;Bin<NumOfBin;Bin++)
	{
   ...
   //Draw, Rt
	...
	const char* file_Name_Rt[3] = {"EH1_Rt_GapRemove.eps","EH2_Rt_GapRemove.eps","EH3_Rt_GapRemove.eps"};
	TCanvas *myC_Rt[3];
	TGraphErrors *gr_Rt[3];

	TLegend *leg_Rt[3];
	const char* leg_Name_Rt[3] = {"EH1_R(t)-R0_MC(gap removed)","EH2_R(t)-R0_MC(gap removed)","EH3_R(t)-R0_MC(gap removed)"};
...
//Draw, _abn
	...
	const char* file_Name_abn[3] = {"EH1_abn_MC_GapRemove.eps","EH2_abn_MC_GapRemove.eps","EH3_abn_MC_GapRemove.eps"};
	TCanvas *myC_abn[3];

	TGraphErrors *gr_an[3];

	TLegend *leg_an[3];
	const char* leg_Name_an[3] = {"EH1_a_{n}_MC(gap removed)","EH2_a_{n}_MC(gap removed)","EH3_a_{n}_MC(gap removed)"};
	TGraphErrors *gr_bn[3];

	TLegend *leg_bn[3];
	const char* leg_Name_bn[3] = {"EH1_b_{n}_MC(gap removed)","EH2_b_{n}_MC(gap removed)","EH3_b_{n}_MC(gap removed)"};
...
//save to .root file
	 ...
	 double an_t[3][2];
	 TFile  *F_an = new TFile("an_MC_GapRemove.root","recreate");
	 ...
	 double bn_t[3][2];
	 TFile  *F_bn = new TFile("bn_MC_GapRemove.root","recreate");
