...
*/
  ...
  */
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /////////////added by ZZC on 2014-10-28///////////////
  //To calculate each hourly hall's prediction and hourly alpha,beta
  ...
  Double_t Z_Alpha[Z_NumOfBin]={0.0};
  Double_t Z_Beta[Z_NumOfBin]={0.0};
  Double_t Z_Fraction[3][6][Z_NumOfBin];// Z_Fraction[Hall][Rct][Bin] = \sum_{AD}{Flux[Rct]*DAQLivetime[AD]/L[AD][Rct]^2}
  memset(Z_Fraction,0.0,sizeof(Z_Fraction));

  Double_t Z_A[Z_NumOfBin]={0.0};
  Double_t Z_B[Z_NumOfBin]={0.0};
  Double_t Z_F[Z_NumOfBin]={0.0};
  ...
  for(int Bin=0;Bin<Z_NumOfBin;Bin++)
  {
	  ...
	  for(int Rct=0;Rct<6;Rct++)
	  {
		  ...
		  {
		  Z_Fraction[0][Rct][Bin] += reactorRate[N_Day][Rct]*Z_FullTime[Det][Bin]/pow(baseline[Det][Rct],2.0);
		  }
		  for(int Det=2;Det<4;Det++)
		  {
		  Z_Fraction[1][Rct][Bin] += reactorRate[N_Day][Rct]*Z_FullTime[Det][Bin]/pow(baseline[Det][Rct],2.0);
		  }
		  for(int Det=4;Det<8;Det++)
		  {
		  Z_Fraction[2][Rct][Bin] += reactorRate[N_Day][Rct]*Z_FullTime[Det][Bin]/pow(baseline[Det][Rct],2.0);
		  }
	  }
  }
  ...
  TFile  *F_AlphaBeta = new TFile("AlphaBeta_FFT.root","recreate");
  ...
  //hourly hall's prediction
  TFile  *F_Prediction = new TFile("Prediction_FFT.root","recreate");
  TTree *Tree_Prediction = new TTree("Prediction","Prediction");
  Tree_Prediction->Branch("Prediction",Prediction_t,"Prediction_t[3]/D");

  for(int Bin=0; Bin<Z_NumOfBin;Bin++)
	{
		Prediction_t[0] = Z_A[Bin];
		Prediction_t[1] = Z_B[Bin];
		Prediction_t[2] = Z_F[Bin];
		Tree_Prediction->Fill();
	}
  Tree_Prediction->Write();
