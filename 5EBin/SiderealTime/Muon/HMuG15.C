...
void HMuG15()
{
	...
	const char *file_Name[8] = {
		"HMuG15InOneDay_EH1_AD1.pdf","HMuG15InOneDay_EH1_AD2.pdf","HMuG15InOneDay_EH2_AD1.pdf","HMuG15InOneDay_EH2_AD2.pdf",
		"HMuG15InOneDay_EH3_AD1.pdf","HMuG15InOneDay_EH3_AD2.pdf","HMuG15InOneDay_EH3_AD3.pdf","HMuG15InOneDay_EH3_AD4.pdf"};
	TCanvas *myC[8];
	TH1F *my_h[8];
	TLegend *leg[8];
	for(int Det=0;Det<8;Det++)
	{
	   ...
	   Double_t MaxR=0.0,MeanR=0.0,MinR=86400.0*HMuG15InOneDay[Det][0]/FullTimeInOneDay[Det][0],MaxE=0.0;

	    for(int i=0;i<24;i++)
	    {
		   my_h[Det]->SetBinContent(i+1,86400.0*HMuG15InOneDay[Det][i]/FullTimeInOneDay[Det][i]);
		   my_h[Det]->SetBinError(i+1,86400.0*sqrt(HMuG15InOneDay[Det][i])/FullTimeInOneDay[Det][i]);
		   MeanR += 86400.0*HMuG15InOneDay[Det][i]/FullTimeInOneDay[Det][i];
		   if(86400.0*HMuG15InOneDay[Det][i]/FullTimeInOneDay[Det][i]>MaxR){MaxR=86400.0*HMuG15InOneDay[Det][i]/FullTimeInOneDay[Det][i];}
		   if(86400.0*HMuG15InOneDay[Det][i]/FullTimeInOneDay[Det][i]<MinR){MinR=86400.0*HMuG15InOneDay[Det][i]/FullTimeInOneDay[Det][i];}
		   if(1.0/sqrt(HMuG15InOneDay[Det][i]) > MaxE){MaxE = 1.0/sqrt(HMuG15InOneDay[Det][i]);}
	    }
		cout<<"Total HMuG15 of "<<hist_Name[Det]<<TotalHMuG15[Det]<<endl;
	...
	my_h[Det]->GetYaxis()->SetTitle("1500~2500MeV Muon Rate (/day)");
   ...
   leg[Det]->Draw();
	...
	//save
	TFile  *F_HMuG15InOneDay = new TFile("HMuG15InOneDay.root","recreate");
	TTree *Tree_HMuG15InOneDay = new TTree("HMuG15InOneDay","HMuG15InOneDay");
	Tree_HMuG15InOneDay->Branch("HMuG15InOneDay",HMuG15_t,"HMuG15_t[8]/D");

	for(int Bin=0; Bin<24;Bin++)
	{
		for(int Det=0;Det<8;Det++)
		{
			HMuG15_t[Det] = HMuG15InOneDay[Det][Bin];
		}
		Tree_HMuG15InOneDay->Fill();
	}
	Tree_HMuG15InOneDay->Write();
