...
void IBD()
{
	...
	for(int Bin=0;Bin<NumOfBin;Bin++)
	{
		...
		for(int Det=0;Det<8;Det++)
		{
			...
			{
			IBDInOneDay[Det][N24][Eidx] += 1.0*IBD_t[Det][Eidx];
			TotalIBD[Det][Eidx] += 1.0*IBD_t[Det][Eidx];
			}
	...
	const char *file_Name[8] = {
		"IBDInOneDay_EH1_AD1.pdf","IBDInOneDay_EH1_AD2.pdf","IBDInOneDay_EH2_AD1.pdf","IBDInOneDay_EH2_AD2.pdf",
		"IBDInOneDay_EH3_AD1.pdf","IBDInOneDay_EH3_AD2.pdf","IBDInOneDay_EH3_AD3.pdf","IBDInOneDay_EH3_AD4.pdf"};
	TCanvas *myC[8];
	TH1F *my_h[8];
	TLegend *leg[8];
	for(int Det=0;Det<8;Det++)
	{
	    ...
	    for(int i=0;i<24;i++)
	    {
		   my_h[Det]->SetBinContent(i+1,86400.0*IBDInOneDay[Det][i]/FullTimeInOneDay[Det][i]);
		   my_h[Det]->SetBinError(i+1,86400.0*sqrt(IBDInOneDay[Det][i])/FullTimeInOneDay[Det][i]);
		   MeanR += 86400.0*IBDInOneDay[Det][i]/FullTimeInOneDay[Det][i];
		   if(86400.0*IBDInOneDay[Det][i]/FullTimeInOneDay[Det][i]>MaxR){MaxR=86400.0*IBDInOneDay[Det][i]/FullTimeInOneDay[Det][i];}
		   if(86400.0*IBDInOneDay[Det][i]/FullTimeInOneDay[Det][i]<MinR){MinR=86400.0*IBDInOneDay[Det][i]/FullTimeInOneDay[Det][i];}
		   if(1.0/sqrt(IBDInOneDay[Det][i]) > MaxE){MaxE = 1.0/sqrt(IBDInOneDay[Det][i]);}
	    }
	...
	my_h[Det]->GetYaxis()->SetTitle("IBD candidate Rate (/day)");
	...
	*/
	//save
	Double_t IBD_t2[8][5];
	TFile  *F_IBDInOneDay = new TFile("IBDInOneDay.root","recreate");
	TTree *Tree_IBDInOneDay = new TTree("IBDInOneDay","IBDInOneDay");
	Tree_IBDInOneDay->Branch("IBDInOneDay",IBD_t2,"IBD_t2[8][5]/D");
	...
	for(int Bin=0; Bin<24;Bin++)
	{
		for(int Det=0;Det<8;Det++)
		{
			for(int Eidx=0;Eidx<5;Eidx++)
			{
			IBD_t2[Det][Eidx] = IBDInOneDay[Det][Bin][Eidx];
			//cout<<IBD_t2[Det]<<endl;
			}
		}
		Tree_IBDInOneDay->Fill();
	}
	Tree_IBDInOneDay->Write();
